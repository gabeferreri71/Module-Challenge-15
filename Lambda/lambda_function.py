...
def validate_data(age, dollars, intent_request):
    ...
    # Validate the amount for age where 0 < age < 65
    if age is not None:
        age = parse_int(age)  
        ...
        if age <0 or age >65:
            return build_validation_result(
                False,
                "age",
                "The person must be between 0 and 65",    
            )
        
    # Validate the amount entered, it should be > 5000
    if dollars is not None:
        dollars = parse_int(dollars)  
...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        # the elicit slot prompt action is used to re-prompt for the first violation detected.
        if not validation_result["isValid"]:
            slots[validation_result["violatedSlot"]] = None  # Cleans invalid slot

            # Returns an elicit slot prompt to request new data for the invalid slot
            return elicit_slot(
